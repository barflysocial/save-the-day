...
function resetSessionStatuses(options = [], targetId = '') {
  return sanitizeSessionOptions(options).map(option => {
    if (!targetId || option.id === targetId) {
...
app.post('/api/checkin', async (req, res) => {
  ...
  try {
    ...
    if (option.status === 'closed') return res.status(409).json({ error: 'Check-in is closed for this session.' });
    if (option.status === 'started') return res.status(409).json({ error: 'This session has already started. Ask the host for help.' });
...
app.post('/api/host/session-status', async (req, res) => {
  if (!requireHostPin(req, res)) return;
  const sessionChoiceId = safeText(req.body.sessionChoiceId, 40);
  const nextStatus = cleanStatus(req.body.status || 'open');
  if (!sessionChoiceId) return res.status(400).json({ error: 'Choose a session to update.' });
  try {
    const current = await query('SELECT session_options FROM sessions WHERE id = $1', [SESSION_ID]);
    const options = sanitizeSessionOptions(current.rows[0]?.session_options || DEFAULT_SESSION_OPTIONS);
    const updated = options.map(option => option.id === sessionChoiceId ? { ...option, status: nextStatus } : option);
    await query('UPDATE sessions SET session_options = $1::jsonb, updated_at = NOW() WHERE id = $2', [JSON.stringify(updated), SESSION_ID]);
    res.json(await getSession());
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.delete('/api/session/rsvps', async (req, res) => {
  ...
  try {
    const current = await query('SELECT session_options FROM sessions WHERE id = $1', [SESSION_ID]);
    const resetOptions = resetSessionStatuses(current.rows[0]?.session_options || DEFAULT_SESSION_OPTIONS);
    ...
    await query('UPDATE sessions SET session_options = $1::jsonb, updated_at = NOW() WHERE id = $2', [JSON.stringify(resetOptions), SESSION_ID]);
    res.json(await getSession());
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.delete('/api/session/selected', async (req, res) => {
  ...
  try {
    const current = await query('SELECT session_options FROM sessions WHERE id = $1', [SESSION_ID]);
    const resetOptions = resetSessionStatuses(current.rows[0]?.session_options || DEFAULT_SESSION_OPTIONS, sessionChoiceId);
...
initDb()
  .then(() => {
    app.listen(PORT, () => console.log(`Save the Day V16 running on port ${PORT}`));
  })
  .catch(error => {
    console.error('Database initialization failed:', error.message);
    app.listen(PORT, () => console.log(`Save the Day V16 running without database on port ${PORT}`));
  });
