...
app.get('/api/host/players', async (req, res) => {
  ...
  try {
    ...
    res.json({ players: players.rows, summary: results.rows[0] });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
initDb()
  .then(() => {
    app.listen(PORT, () => console.log(`Save the Day V6 running on port ${PORT}`));
  })
  .catch(error => {
    console.error('Database initialization failed:', error.message);
    app.listen(PORT, () => console.log(`Save the Day V6 running without database on port ${PORT}`));
  });
