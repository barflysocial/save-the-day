# Save the Day V13 — Clean Title Continue

V13 makes the three sessions truly separate while keeping one general public player link.
...
1. Upload the unzipped V13 folder to GitHub.
...
V13 updates: host can upload a venue/event logo, preview it in host setup, remove it, and display it inside the Event Details card on the player title screen and related player screens.


V13 updates: new clean title screen image is used as the locked title screen, intro fades to the title and holds for 5 seconds, Continue popup opens the action panel, Choose Session is a dropdown, My RSVP was added, RSVP uses Save RSVP / RSVP Confirmed flow, global Home is hidden on the title screen, and explicit /player + /host routes were added for refresh-safe deployment.
