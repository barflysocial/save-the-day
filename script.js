...
function clearAllLocalSessionData() {
  [RSVP_LIST_KEY, RSVP_KEY, PLAYER_KEY, PLAYER_ID_KEY, CHECKIN_KEY, CHECKIN_LIST_KEY, SESSION_CHOICE_KEY, SHARE_CLICKS_KEY, ...LEGACY_KEYS].forEach(key => localStorage.removeItem(key));
  getSessionOptions().forEach(option => localStorage.removeItem(`${SHARE_CLICKS_KEY}:${option.id}`));
  localStorage.removeItem(`${SHARE_CLICKS_KEY}:general`);
}

function clearLocalSelectedSessionData(sessionId) {
  if (!sessionId) return;
  const rsvps = getRsvpList().filter(player => player.sessionChoiceId !== sessionId);
  safeWriteStorage(RSVP_LIST_KEY, rsvps);
  const checkins = (safeReadStorage(CHECKIN_LIST_KEY, []) || []).filter(item => item.sessionChoiceId !== sessionId);
  safeWriteStorage(CHECKIN_LIST_KEY, checkins);
  safeWriteStorage(`${SHARE_CLICKS_KEY}:${sessionId}`, 0);
  const current = getCurrentRsvp();
  if (current?.sessionChoiceId === sessionId) {
    [RSVP_KEY, PLAYER_KEY, PLAYER_ID_KEY, CHECKIN_KEY, SESSION_CHOICE_KEY].forEach(key => localStorage.removeItem(key));
  }
}

function resetLocalSessionStatuses(targetId = '') {
  const settings = getHostSettings();
  settings.sessionOptions = normalizeSessionOptions(settings.sessionOptions || DEFAULT_SESSION_OPTIONS).map(option => {
    if (!targetId || option.id === targetId) return { ...option, status: 'open', computedStatus: 'open' };
    return option;
  });
  saveHostSettings(settings);
  sessionCache.settings = settings;
}

async function refreshHostAfterReset(message) {
  await refreshSession();
  renderHostSessionTools();
  fillEventSummaries();
...
async function clearSession() {
  const ok = window.confirm('Clear ALL session data? This clears RSVPs, check-ins, results, share clicks, and resets every session back to Open.');
  if (!ok) return;
  const status = $('#host-status');
  clearTimer();
  clearClueTimer();
  ...
  if (sessionCache.backendReady) {
    try {
      if (status) status.textContent = 'Clearing all live session data...';
    ...
    resetLocalSessionStatuses();
  }

  clearAllLocalSessionData();
  resetCurrentPlayerGame();
  await refreshHostAfterReset('All sessions reset. RSVPs, check-ins, results, and share clicks are back to 0. Session statuses are Open.');
}
...
async function clearSelectedSession() {
  ...
  const ok = window.confirm(`Reset ${option.label}? This clears RSVPs, check-ins, shares, results, and sets the session back to Open.`);
  if (!ok) return;
  const status = $('#host-status');
  clearTimer();
  clearClueTimer();
  ...
  if (sessionCache.backendReady) {
    try {
      if (status) status.textContent = `Resetting ${option.label}...`;
    ...
    clearLocalSelectedSessionData(selectedId);
    resetLocalSessionStatuses(selectedId);
  }

  clearLocalSelectedSessionData(selectedId);
  resetCurrentPlayerGame();
  await refreshHostAfterReset(`${option.label} reset. RSVPs, check-ins, shares, and results are cleared and the session is Open.`);
}

async function restoreOriginalState() {
  const ok = window.confirm('Restore original state? This clears RSVPs, check-ins, results, share clicks, player progress, logo, and host date/time/location settings.');
  if (!ok) return;
  const status = $('#host-status');
  clearTimer();
  clearClueTimer();
  ...
  if (sessionCache.backendReady) {
    try {
      if (status) status.textContent = 'Restoring live database session...';
  ...
  clearAllLocalSessionData();
  [HOST_SETTINGS_KEY, HOST_VIEW_MODE_KEY].forEach(key => localStorage.removeItem(key));
  resetCurrentPlayerGame();
  if (isHostRoute()) {
    await renderHost();
    const restoredStatus = $('#host-status');
    if (restoredStatus) restoredStatus.textContent = 'Original state restored. All sessions, counts, status, logo, and player data are reset.';
  } else {
    renderTitle();
  }
}
