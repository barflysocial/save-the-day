...
const PLAYER_PATH = '/save-the-day';
const HOST_PATH = '/save-the-day/host';
const CHECKIN_KEY = 'saveTheDayCurrentCheckinV9';
...
function fillEventSummaries() {
  ...
  if (checkinSummary) checkinSummary.innerHTML = eventDetailsMarkup(settings);
  if (shareSummary) shareSummary.innerHTML = eventDetailsMarkup(settings);
  if (hostLoginSummary) hostLoginSummary.innerHTML = eventDetailsMarkup(settings);
  const playerUrlDisplay = $('#player-url-display');
  if (playerUrlDisplay) playerUrlDisplay.textContent = getPlayerUrl();
...
async function submitCheckIn() {
  ...
  if (sessionCache.backendReady) {
    try {
      ...
      const data = await apiJson('/api/checkin', {
        method: 'POST',
        body: JSON.stringify({ instagram, gameCode })
      });
      const player = {
        id: data.player.id,
      ...
      safeWriteStorage(CHECKIN_KEY, { instagram: player.instagram, checkedInAt: new Date().toISOString() });
      sessionCache.checkinCount = Number(data.checkinCount || sessionCache.checkinCount);
  ...
  if (!checkins.find(c => c.instagram === instagram)) {
    checkins.push({ instagram, checkedInAt: new Date().toISOString() });
    safeWriteStorage(CHECKIN_LIST_KEY, checkins);
  }
  safeWriteStorage(CHECKIN_KEY, { instagram, checkedInAt: new Date().toISOString() });
  safeWriteStorage(PLAYER_KEY, player);
  state.accessGranted = true;
  startGame();
}
...
async function recordShareClick(channel = 'title') {
  ...
  const count = Number(safeReadStorage(SHARE_CLICKS_KEY, 0) || 0) + 1;
  safeWriteStorage(SHARE_CLICKS_KEY, count);
  sessionCache.shareClickCount = count;
}
...
async function renderShareGame() {
  ...
  $('#share-player-url').textContent = url;
  $('#share-text-preview').textContent = text;
  const qr = $('#player-qr');
  if (qr) qr.src = `https://api.qrserver.com/v1/create-qr-code/?size=280x280&data=${encodeURIComponent(url)}`;
  const status = $('#share-status');
  if (status) status.textContent = `Share click counted. Total tracked clicks: ${sessionCache.shareClickCount || 0}.`;
  bindGlobalActions();
}
...
async function renderAccessGate() {
  ...
  if (current) {
    ...
    $('#confirmed-copy').textContent = `${current.name} ${current.instagram ? `(${current.instagram})` : ''} has RSVP'd. Use the Check In button on the title screen when the host gives the game code.`;
  } else if (full) {
...
async function loadHostRoster() {
  ...
  if (!sessionCache.backendReady) {
    ...
    const checkins = safeReadStorage(CHECKIN_LIST_KEY, []);
    const shares = Number(safeReadStorage(SHARE_CLICKS_KEY, 0) || 0);
    if ($('#host-rsvp-count')) $('#host-rsvp-count').textContent = localList.length;
    if ($('#host-checkin-count')) $('#host-checkin-count').textContent = Array.isArray(checkins) ? checkins.length : 0;
    if ($('#host-share-count')) $('#host-share-count').textContent = shares;
    if (live) live.textContent = `Offline fallback: ${localList.length} local RSVP(s), ${Array.isArray(checkins) ? checkins.length : 0} check-in(s), ${shares} share click(s).`;
    if (list) list.innerHTML = localList.map(player => `<div class="roster-row"><span>${player.slot}. ${player.name}<small>${player.instagram}</small></span><strong>${Array.isArray(checkins) && checkins.find(c => c.instagram === player.instagram) ? 'Checked In' : 'RSVP'}</strong></div>`).join('') || '<p class="small-hint">No local RSVPs yet.</p>';
    return;
  }
  try {
    ...
    if (live) live.textContent = `${data.summary.rsvp_count || data.players.length || 0} RSVP(s). ${data.summary.checkin_count || 0} checked in. ${data.summary.share_click_count || 0} share click(s). ${data.summary.finished_count || 0} finished. Average score: ${data.summary.average_score || 0}.`;
    if (list) {
      list.innerHTML = data.players.map(player => {
        const status = player.rank ? `${player.total_score}/10 • ${player.rank}` : (player.checked_in_at ? 'Checked In' : 'RSVP');
        return `
        <div class="roster-row">
          <span>${player.slot}. ${player.name} <small>${player.instagram}${player.checked_in_at ? ' • checked in' : ''}</small></span>
...
function initApp() {
  if (isHostRoute()) {
    ...
    if (window.location.pathname !== PLAYER_PATH && window.history?.replaceState) window.history.replaceState({}, '', PLAYER_PATH);
    renderIntro();
  }
}
