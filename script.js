...
function getSessionOptions(settings = getHostSettings()) {
  return normalizeSessionOptions(settings.sessionOptions || DEFAULT_SESSION_OPTIONS);
}
...
function getSelectedSession() {
  ...
  if (saved) {
    const fresh = options.find(option => option.id === saved.id);
...
function renderSessionPicker(containerId, readOnly = false) {
  ...
  container.innerHTML = options.map(option => {
    ...
    const spots = `${option.spotsLeft ?? Math.max(0, option.maxPlayers - option.rsvpCount)} spots left`;
    const counts = `${option.rsvpCount || 0}/${option.maxPlayers || MAX_PLAYERS} RSVPs • ${option.checkinCount || 0} checked in`;
    return `
      <div class="session-choice-card ${chosen ? 'selected' : ''} ${disabled ? 'disabled-session' : ''}">
        <div class="session-meta">
          <strong>${option.label}</strong>
          <span>${[formatDisplayDate(option.date), formatDisplayTime(option.time)].filter(Boolean).join(' • ') || 'Date and time coming soon'}</span>
          <small>${counts} • ${spots}</small>
          <em>${status}</em>
        </div>
        ${readOnly ? `<span class="session-selected-pill">${chosen ? 'Selected' : status}</span>` : `<button class="ghost-btn small" type="button" data-action="pick-session" data-session-id="${option.id}" ${disabled ? 'disabled' : ''}>${chosen ? 'Selected' : 'Choose'}</button>`}
      </div>
    `;
  }).join('');
}
...
async function renderHost() {
  ...
  const hostSessions = getSessionOptions(settings);
